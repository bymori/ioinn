...
- 字面量--字面上显示的东西  var str='ioinn' var num=1
...
1. 万物皆对象,万物皆空-> 指向null
2. 两个定义:
   1. 原型:保存所有子对象的共有属性值和方法的父对象
   2. 原型链:由各级子对象的`__proto__`属性连续引用行成的结构
...
![image](https://cdn.jsdelivr.net/gh/bymori/image-PicX@main/blog/image.4qnjvcl32ok.png)
Function对象和Object对象之间的关系 Person Aa
Function是顶层的构造器 Object是顶层的对象{}
顶层现有`null`,`Object.prototype`,`Function.prototype` `Function`

从原型上说:Function继承了Object toString
从构造器上说:Function构造了Object new Object()
`function Object(){}`

原型相关的API判断对象的属性是自有的还是私有的
...
3. **组合继承(最常用)**：类式继承+构造函数继承
4. **寄生组合继承(最佳方式)**:寄生式继承+构造函数式继承
5. **原型式**：跟类式继承一样，父类对象hook中的值类型的属性被赋值，引用类型的属性被共有
6. **寄生式**：通过在一个函数内的过度对象实现继承并返回新对象的方式

### 开发中如何运用 Object.defineProperty() 方法

**Object.defineProperty()**方法 应用场景
