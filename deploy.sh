...
###
 ...
 # @LastEditors: by_mori
 # @LastEditTime: 2021-11-02 21:47:04
### 
...
if [ -z "$CODING_TOKEN" ]; then  # -z 字符串 长度为0则为true；$CODING_TOKEN来自于github仓库`Settings/Secrets`设置的私密环境变量
  ...
  codingUrl=git@e.coding.net:bymori/ioinn.git
else
  #注意！！！这里需要使用coding提供的个人令牌的用户名和token
  codingUrl=https://ioinn:${CODING_TOKEN}@e.coding.net:bymori/ioinn.git
fi
git add -A
git commit -m "${msg}"
git push -f $codingUrl master # 推送到 coding
